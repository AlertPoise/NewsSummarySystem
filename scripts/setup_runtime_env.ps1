...
New-Item -ItemType Directory -Force -Path $runtimeDir|Out-Null; Set-Content -LiteralPath $logFile -Value ("Setup started: {0:u}" -f (Get-Date)) -Encoding UTF8
...
function Run([string]$label,[string]$file,[string[]]$arguments,[string]$hint=''){
 Log "      $label"; & $file @arguments 2>&1|ForEach-Object{Write-Host $_;Add-Content -LiteralPath $logFile -Value $_ -Encoding UTF8}
 if($LASTEXITCODE -ne 0){throw "$label failed (exit code $LASTEXITCODE). $hint"}
}
...
function Get-Torch([string]$p){$o=& $p -c 'import torch;print(torch.__version__);print(torch.version.cuda or "cpu")' 2>$null;if($LASTEXITCODE -ne 0 -or $o.Count -lt 2){return $null};return [PSCustomObject]@{Version=([string]$o[0]).Trim();Cuda=([string]$o[1]).Trim()}}
...
try{
 ...
 $probe=& $runtimePython -c 'import platform,sys;print(sys.executable);print(sys.version_info[0]);print(sys.version_info[1]);print(platform.architecture()[0])' 2>&1
 $pt=$probe -join "`n"
 if($LASTEXITCODE -ne 0){Fail "根目录 .venv 无法执行：$runtimePython"}
 ...
 Log '[3/7] GPU 检测';$gpu=$false;$gpuName='N/A';$driver='N/A';$driverCuda=$null;$smi=Get-Command nvidia-smi -EA SilentlyContinue;if($smi){$q=& $smi.Source --query-gpu=name,driver_version --format=csv,noheader 2>$null;$txt=(& $smi.Source 2>$null)-join "`n";if($LASTEXITCODE -eq 0 -and $q){$parts=([string]($q|Select-Object -First 1))-split ',\s*',2;$gpuName=$parts[0].Trim();if($parts.Count -gt 1){$driver=$parts[1].Trim()};if($txt -match 'CUDA Version:\s*([0-9]+\.[0-9]+)'){$driverCuda=[version]$Matches[1]};$gpu=$true}};Log "      GPU detected: $gpu";Log "      GPU name: $gpuName";Log "      Driver version: $driver";Log "      Driver CUDA capability: $driverCuda"
